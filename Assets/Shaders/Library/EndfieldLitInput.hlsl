#ifndef ENDFIELD_LIT_INPUT_INCLUDED
#define ENDFIELD_LIT_INPUT_INCLUDED
...
half3 _HeadRight;
half3 _HeadForward;
half3 _FaceSideColor;
half _SceneShadowCenter;
half _SceneShadowSharpness;
half _HalfLambertShadowCenter;
half _HalfLambertShadowSharpness;
half _SDFSharpness;
half _SDFShadowCenter;

half3 _OutlineColor;
half _OutlineWidth;
half _OutlineClipSpaceZOffset;

half3 _AnisotropicHLColor;
half _AnisotropicHLIntensity;

half _DirectLightingIntensity;
half _GIIntensity;
CBUFFER_END
...
TEXTURE2D(_SpecularRampMap); SAMPLER(sampler_SpecularRampMap);
TEXTURE2D(_PMaskMap); SAMPLER(sampler_PMaskMap);
...
TEXTURE2D(_ST); SAMPLER(sampler_ST);
...
inline void InitializeStandardLitSurfaceData(float2 uv, out SurfaceData outSurfaceData)
{
    ...
    half4 pbrMask = SAMPLE_TEXTURE2D(_PMaskMap, sampler_PMaskMap, uv);

#ifdef _RENDERTYPE_CLOTH
    outSurfaceData.metallic = pbrMask.r;
    outSurfaceData.smoothness = pbrMask.a;
    outSurfaceData.occlusion = pbrMask.b;
#elif defined(_RENDERTYPE_SKIN)
    outSurfaceData.metallic = 0.0;
    outSurfaceData.smoothness = 1.0;
    outSurfaceData.specular = 1.0;
    outSurfaceData.occlusion = albedoAlpha.a;
#elif defined(_RENDERTYPE_FACE)
    outSurfaceData.metallic = 0.0;
    outSurfaceData.smoothness = 0.3;
